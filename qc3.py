...
# Specify the backend session and mode
backend = service.backend('ibm_kyoto')  # Or specify another real backend if needed
...
def cost_function(params):
    ...
    job = sampler.run([qc], shots=rep)  # Notice the circuit is wrapped in a list
    result = job.result()
...
sampler = Sampler(session=backend, mode='simulator')
job = sampler.run([qc], shots=rep)  # Again, wrap qc in a list
result = job.result()
counts = result.get_counts(qc)
